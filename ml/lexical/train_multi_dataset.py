...
def load_sarc_dataset():
    ...
    try:
        ...
        df_train = pd.read_csv(train_file, header=None, usecols=[0, 1], 
                               names=['label', 'comment'], nrows=100000)  # Limit for memory
...
def load_custom_conversational():
    ...
    print(f"   Added {len(texts)} custom examples ({len(sarcastic_examples)} sarcastic)")
...
def main():
    ...
    print(f"   Not Sarcastic: {len(all_labels)-sum(all_labels):,} ({(1-sum(all_labels)/len(all_labels))*100:.1f}%)")
