...
def main():
    ...
    print(f"   Not Sarcastic: {len(labels)-sum(labels):,} ({(1-sum(labels)/len(labels))*100:.1f}%)")
    ...
    for tfidf_name, clf_name in test_combinations:
        pipeline = Pipeline([
            ('tfidf', tfidf_configs[tfidf_name]),
    ...
    lr_accuracy = evaluate_model(best_lr_model, X_test, y_test, "Tuned Logistic Regression")
    ...
    svm_accuracy = evaluate_model(calibrated_svm, X_test, y_test, "Tuned & Calibrated LinearSVC")
    ...
    if 'Ensemble' not in best_name and ensemble_accuracy >= best_acc - 0.005:
        print(f"   📌 Using Ensemble instead (within 0.5% of best, more robust)")
        final_model = ensemble
