...
def load_models(verbose: bool = False):
    ...
    if not MODEL_PATH.exists():
        raise FileNotFoundError(f"Classifier not found at {MODEL_PATH}. Run train_prosodic.py first.")
...
def load_audio(audio_path: str, verbose: bool = False) -> tuple:
    ...
    if verbose:
        print(f"  Original: {waveform.shape[1]} samples at {sr}Hz ({waveform.shape[1]/sr:.2f}s)")
    ...
    if verbose:
        print(f"  Processed: {len(waveform)} samples ({len(waveform)/TARGET_SAMPLE_RATE:.2f}s)")
    
    return waveform


def extract_embedding(waveform: torch.Tensor, processor, model, verbose: bool = False) -> np.ndarray:
...
def main():
    ...
    parser.add_argument(
        "audio_files",
        nargs="+",
        help="Path(s) to audio file(s) to analyze"
    )
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
