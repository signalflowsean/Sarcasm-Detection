...
def evaluate_with_cross_validation(
    X: np.ndarray, y: np.ndarray, n_folds: int = 5, C: float = 1.0, class_weight=None
):
    """
...
def train_final_model(
    X: np.ndarray, y: np.ndarray, C: float = 1.0, class_weight=None
) -> Pipeline:
    """
...
def evaluate_holdout(
    ...
    print(
        classification_report(
            y_test, y_pred, target_names=["Non-Sarcastic", "Sarcastic"]
        )
    )
...
def test_predictions(pipeline: Pipeline, df: pd.DataFrame, n_samples: int = 5):
    ...
    sarcastic_df = df[df["label"] == 1].sample(
        n=min(n_samples, (df["label"] == 1).sum()), random_state=42
    )
    non_sarcastic_df = df[df["label"] == 0].sample(
        n=min(n_samples, (df["label"] == 0).sum()), random_state=42
    )
...
def main():
    ...
    cv_scores = evaluate_with_cross_validation(
        X, y, n_folds=5, C=best_C, class_weight=best_class_weight
    )
    ...
    print("Update backend/app.py to load and use this model.")
