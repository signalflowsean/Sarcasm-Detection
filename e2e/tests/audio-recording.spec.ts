...
async function injectAudioMocks(page: Page, audioBase64: string) {
  await page.addInitScript(
    ({ audioData }) => {
      ...
      ;(window as unknown as { __testAudioBase64: string }).__testAudioBase64 = audioData
      ...
      class MockMediaRecorder {
        ...
        constructor(stream: MediaStream, options?: { mimeType?: string }) {
          ...
          console.log('[E2E Mock] MockMediaRecorder created with mimeType:', this.mimeType)
        }
        ...
        stop() {
          console.log('[E2E Mock] MockMediaRecorder.stop() called, current state:', this.state)
          ...
          setTimeout(() => {
            ...
            const base64 = (window as unknown as { __testAudioBase64: string }).__testAudioBase64
            ...
            console.log('[E2E Mock] Delivering audio blob:', blob.size, 'bytes')
        ...
        static isTypeSupported(mimeType: string): boolean {
          return ['audio/webm', 'audio/webm;codecs=opus', 'audio/ogg', 'audio/wav', 'audio/mp4'].some(
            type => mimeType.startsWith(type.split(';')[0])
          )
        }
      }
...
test.describe('Audio Recording', () => {
  ...
  test('should discard recording when discard button clicked', async ({ page }) => {
    await page.goto('/audio-input')
    await page.waitForTimeout(500)
  ...
  test('should use keyboard shortcut R to toggle recording', async ({ page }) => {
    await page.goto('/audio-input')
...
test.describe('Audio Recording - Error States', () => {
  ...
  test.skip('should handle getUserMedia rejection gracefully', async ({ page }) => {
