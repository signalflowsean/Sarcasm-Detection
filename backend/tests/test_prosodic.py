...
import pytest
from tests.mocks import create_mock_wav_audio, create_invalid_audio, create_empty_file
...
def test_prosodic_empty_file_returns_400(client):
    ...
    data = {
        'audio': (create_empty_file(), 'test.wav', 'audio/wav')
    }
...
def test_prosodic_invalid_audio_returns_400(client):
    ...
    data = {
        'audio': (create_invalid_audio(), 'test.wav', 'audio/wav')
    }
    response = client.post('/api/prosodic', data=data, content_type='multipart/form-data')
...
def test_prosodic_invalid_extension_returns_400(client):
    ...
    data = {
        'audio': (create_mock_wav_audio(), 'test.txt', 'text/plain')
    }
    response = client.post('/api/prosodic', data=data, content_type='multipart/form-data')
...
def test_prosodic_valid_wav_returns_200(client):
    ...
    data = {
        'audio': (create_mock_wav_audio(), 'test.wav', 'audio/wav')
    }
    response = client.post('/api/prosodic', data=data, content_type='multipart/form-data')
