...
const Indicator = ({
  ...
  const fill = enabled
    ? (variant === 'prosodic' ? 'url(#prosodic-gradient)' : 'url(#lexical-gradient)')
    : 'url(#disabled-gradient)';
  
  const stroke = enabled
    ? (variant === 'prosodic' ? '#5a1520' : '#5c4020')
    : '#6f6a60';
  ...
  return (
    <g
      ...
      style={{
        ...
        transition: canAnimate ? `transform ${animDuration}ms ease-out, opacity 300ms ease` : 'none',
        transformOrigin: '0 0',
...
const LevelIndicators = ({
  ...
  return (
    <div 
      ref={containerRef}
      className="level-indicators-container"
      aria-hidden="true"
    >
      {dimensions && (
        <svg
